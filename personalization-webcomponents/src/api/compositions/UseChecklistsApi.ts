import {
    ChecklistsApi,
    Configuration, type FetchParams, type Middleware,
...
export function useChecklistsApi() {
    return new ChecklistsApi(new Configuration({
...
const removePublicPrefixMiddleware: Middleware = {
    pre: async (context: FetchParams): Promise<FetchParams | void> => {
        const url = new URL(context.url);

        if (url.pathname.startsWith(PATHNAME_P13N_BACKEND_PUBLIC_ROUTES + '/public')) {
            url.pathname = url.pathname.replace(PATHNAME_P13N_BACKEND_PUBLIC_ROUTES + '/public', PATHNAME_P13N_BACKEND_PUBLIC_ROUTES);
        }
...
export function usePublicServiceNavigatorEndpoints() {
    return new PublicServiceNavigatorEndpointsApi(new Configuration({
