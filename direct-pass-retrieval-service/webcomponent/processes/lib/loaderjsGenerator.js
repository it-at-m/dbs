...
export function generateLoaderJs(filename) {
    ...
    const loaderJsTemplate = fs.readFileSync(
        `${__dirname}/loader.js.template`,
        { encoding: "utf-8" }
    );
    ...
    const vitepressLoaderJsTemplate = fs.readFileSync(
        `${__dirname}/vitepress-loader.js.template`,
        { encoding: "utf-8" }
    );
    const vitepressLoaderJsReplaced = vitepressLoaderJsTemplate.replace(
        "{{filename}}",
        filename
    );
    fs.writeFileSync(
        path.resolve("./dist/vitepress-loader.js"),
        vitepressLoaderJsReplaced,
        { encoding: "utf-8" }
    );
}
