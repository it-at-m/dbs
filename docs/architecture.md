...
## DBS-Ticketing with [Zammad](https://opensource.muenchen.de/de/software/zammad.html) {#ticketing}
...
The checklist is generated by Kaptara Iason based on a query.
The list of services is transferred to our `personalization-webcomponents` via local storage
and citizens can view the calculated services there.
They also have the option to save the checklist in their user profile (`personalization-service`)
and edit it in the `personalization-webcomponents`.

## DBS-Login (Authentication) {#login}
